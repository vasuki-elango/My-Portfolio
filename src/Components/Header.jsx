...
export const Header = () => {
  ...
  return (
    <nav className="w-full bg-zinc-800 flex items-center justify-between px-6 md:px-16 py-4 fixed top-0 z-10">
      {/* Logo */}
      <Link to='intro'  smooth={true} offset={-100} duration={500} className='text-4xl text-yellow-500 font-bold'>Vasuki.</Link>
      ...
      {/* Contact btn */}
      <button className="lg:flex items-center justify-center gap-2 rounded-full px-4 py-2 bg-white transition-all hover:bg-amber-300 hidden text-black" onClick={()=>{
            document.getElementById("contact").scrollIntoView();
          }}>
          <FaMessage />Contact Me
      </button>

      {/* menu icon */}
      <img src={menu} alt="menu" className='lg:hidden w-8'  onClick={()=>setshowMenu(!showMenu)}/>

      {/* mobile navbar */}
      <div className='bg-zinc-700 absolute top-[4.5rem] right-4 flex flex-col w-3/4' style={{display: showMenu? 'flex':'none'}}>
